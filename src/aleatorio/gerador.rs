...
/** Compila todas funções em uma só, que poder emitir quaisquer tipos num 
 * só módulo, onde têm funções com nomes parecidos dos tipos, assim invokar 
 * tal função parece o tipo acompanhado de "()". As funções no só chamam as 
 * funções acimas que já trabalham na parte de gerar tais coisas aleatórias,
 * e algumas também implemenetam 'Range' que as funções acimas foram deixas 
 * de fora nisso. 

 ```
   let a = sortear::u8(100..=243);
   let b = sortear::i8(-12..=2);
   let c = sortear::u16(10_000..=20_000);

   assert!(a >= 100 && a <=243);
   assert!(b >= -12 && b <=2);
   assert!(c >= 10_000 && c <=20_000);
 ```
*/
pub mod sortear {
 ...
 * 
 * Esta técnica é imensamente mais veloz que as demais já tentadas. E vamos
 * realizar os benchmarks aqui. Porém, ela tem um problema padrão por 
 * não ser thread-safe, já que, dois processos forqueados computariam, 
 * o mesmo valor "aleatório", portanto não seria aleatório.
 */

#[allow(dead_code)]
static mut SEMENTE: usize = usize::MAX;
#[allow(dead_code)]
static mut ACIONADO: bool = false;

#[allow(dead_code)]
fn maior_valor_randomico() -> usize {
   /* Está aqui dá ao valor inicial um endereço tirado do próprio program
    * em tempo de execução, o demais valor, a "semente" é modificada 
    * efetuando 64 shifts, distribuídos em direções alternadas, e também 
    * é aplicada uma operação xor depois de cada shift. */
   unsafe {
      // só joga contra o endereço da função uma primeira vez.
      if !ACIONADO { 
         let endereco: usize;
         endereco = lancamento as *const usize as usize;
         SEMENTE ^= endereco; 
         ACIONADO = true;

         if cfg!(debug_assertions)
            { println!("executado apenas uma vez!"); }
      }

      // 64-bits movidos no total.
      SEMENTE ^= SEMENTE << 8;
      SEMENTE ^= SEMENTE >> 12;
      SEMENTE ^= SEMENTE >> 23;
      SEMENTE ^= SEMENTE << 11;
      SEMENTE ^= SEMENTE >> 7;
      SEMENTE ^= SEMENTE << 4;
      // envolve o maior valor possível.
      SEMENTE.rem_euclid(usize::MAX)
      // SEMENTE % usize::MAX
   }
}
...
mod tests {
   ...
   #[test]
   fn inicial_do_novo_gerador() {
      for _ in 1..=27
         { println!("{}", super::maior_valor_randomico()); }
   }

   use std::time::{Duration, Instant};
   use crate::legivel::valor_legivel;

   #[test]
   fn comparacao_com_antigo() {
      let i = Instant::now();
      let mut contagem_novo = 0usize;
      let mut contagem_antigo = 0usize;
      let limite = Duration::from_secs_f32(1.7);

      while i.elapsed() < limite {
         super::maior_valor_randomico(); 
         contagem_novo += 1;
      }

      let i1 = Instant::now();
      while i1.elapsed() < limite {
         super::lancamento();
         contagem_antigo += 1;
      }

      println!(
         "resultado novo: {} sorteios.
         \rresultado antigo: {} sorteios
         \rdiferença com um fator de {} números a mais.",
         valor_legivel(contagem_novo), 
         valor_legivel(contagem_antigo),
         (contagem_novo / contagem_antigo) - 1
      );
   }
}
